...
# sudo touch /var/log/ModuleBalancing
# sudo touch /var/log/CellBalancing
#
...
#-------------------------------------------------------------------------------------------------------------------
SystemSerial_array=( SN000044 SN000068 SN000168 SN000173 SN000187 SN000198 SN000230 SN000245 )
#-------------------------------------------------------------------------------------------------------------------
#-------------------------------------------------------------------------------------------------------------------
...
if [ $MatchSystemSerial -eq	0 ]; then
	echo $SystemSerial ist nicht Teil der Testphase >> ${_LOGFILE_}
	sudo rm -f ${_LOGFILE_}
	sudo rm -f /home/admin/bin/BusinessOptimum.sh
	exit
fi
#===================================================================================================================
#===================================================================================================================
...
while ( [ $System_Initialization = $system_initialization_req ] && [ $System_Running -eq $system_running_req ] ); do
	...
	#============================================================================================================================	
	if ( [ -f /home/admin/registry/out/gen2 ] && [ $bmmType == "sony" ] ); then	
		if ( ( [ $SoC_module_diff -ge 8 ] && [ $SoC_module_diff -le 15 ] && [ $(date +%u) -eq 6 ] && [ $(date +%H) -ge 10 ] && [ $(date +%H) -lt 16 ] && [ $SoC_int -ge 60 ] && [ $SoC_int -le 90 ] ) || ( [ $SoC_module_diff -ge 11 ] && [ $SoC_module_diff -le 15 ] && [ $(date +%H) -ge 10 ] && [ $(date +%H) -lt 16 ] && [ $SoC_int -ge 60 ] && [ $SoC_int -le 90 ] ) || ( [ $SoC_module_diff -ge 16 ] && [ $SoC_module_diff -le 35 ] && [ $(date +%H) -ge 10 ] && [ $(date +%H) -lt 16 ] && [ $SoC_int -ge 50 ] && [ $SoC_int -le 90 ] ) || ( [ $SoC_module_diff -ge 36 ] && [ $SoC_module_diff -le 60 ] && [ $(date +%H) -ge 10 ] && [ $(date +%H) -lt 16 ] && [ $SoC_int -ge 25 ] && [ $SoC_int -le 90 ] ) || ( [ $counter_forced_charging -ge 3 ] && [ $SoC_int -le 90 ] ) ); then		
			ModuleBalancing=0	# Rest ModuleBalancing, - depening on SoC_module_diff certain power ranges will be enabled for charging
			# Display / Print Battery Status
			echo "" >> ${_LOGFILE_}
			...
			if [ $System_Running -ne $system_running_req ]; then
				...
				echo '----------------------------------------------------------' >> ${_LOGFILE_}			
				CPOL1_OffsetDuration_setVal=1000		#Preset OffsetDuration: 1000s only, safety aspects
				CPOL1_offset_time=$(date +%s)							
				CPOL1_OffsetStart_setVal=$CPOL1_offset_time				
			fi
		
			if ( [ $SoC_module_diff -ge 0 ] && [ $SoC_module_diff -le 5 ]  ); then		# when requested due to 3x forced chargding or ModuleBalancing
				ModuleBalancing=1
			fi
			if ( [ $SoC_module_diff -gt 5 ] && [ $SoC_module_diff -le 15 ]  ); then		
				ModuleBalancing=1
			fi	
			if ( [ $SoC_module_diff -gt 15 ] && [ $SoC_module_diff -le 25 ]  ); then		
				ModuleBalancing=2
			fi
			if ( [ $SoC_module_diff -gt 25 ] && [ $SoC_module_diff -le 35 ]  ); then		
				ModuleBalancing=3
			fi
			if ( [ $SoC_module_diff -gt 35 ] && [ $SoC_module_diff -le 100 ]  ); then		
				ModuleBalancing=4
			fi
			...
			while [ $SoC_int -lt 99 ]; do 
					...
					echo $time '||' Cell_Δ: $U_cell_diff_V mV '|' SoC_Δ: $SoC_module_diff % '|' PVundHH: $PVandHH_int W '|' Capa_% $capa_module_100_int % '|' SoC: $SoC_int % '|' ChargedFlag: $ChargedFlag '|' INV: $CPOL1_Mod '|' INV: $Inv_Request W '|' X '|' Module-Balancing $ModuleBalancing '(1)' >> ${_LOGFILE_}		  				
					...
					# Power Determination depending on SoC	
						#-------------------------------------------------------------------------------------------					
						if [ $ModuleBalancing -eq 1 ]; then		# 5% .... 15%
							if [ $SoC_int -lt 80 ]; then
								CPOL1_Wchrg_setMag_f=7000
								CPOL1_Wchrg_setMag_f=$(awk '{print $1+$2}' <<<"${CPOL1_Wchrg_setMag_f} ${SoC_int}")				
							fi													
							if ( [ $SoC_int -ge 80 ] && [ $SoC_int -lt 85 ] ); then
								CPOL1_Wchrg_setMag_f=6000
								CPOL1_Wchrg_setMag_f=$(awk '{print $1+$2}' <<<"${CPOL1_Wchrg_setMag_f} ${SoC_int}")				
							fi						
						fi		
						#-------------------------------------------------------------------------------------------
						if [ $ModuleBalancing -eq 2 ]; then		# 15% .... 25%
							if [ $SoC_int -lt 75 ]; then
								CPOL1_Wchrg_setMag_f=6000
								CPOL1_Wchrg_setMag_f=$(awk '{print $1+$2}' <<<"${CPOL1_Wchrg_setMag_f} ${SoC_int}")				
							fi													
							if ( [ $SoC_int -ge 75 ] && [ $SoC_int -lt 85 ] ); then
								CPOL1_Wchrg_setMag_f=5000
								CPOL1_Wchrg_setMag_f=$(awk '{print $1+$2}' <<<"${CPOL1_Wchrg_setMag_f} ${SoC_int}")				
							fi						
						fi	
						#-------------------------------------------------------------------------------------------
						if [ $ModuleBalancing -eq 3 ]; then		# 25% .... 35%
							if [ $SoC_int -lt 75 ]; then
								CPOL1_Wchrg_setMag_f=5000
								CPOL1_Wchrg_setMag_f=$(awk '{print $1+$2}' <<<"${CPOL1_Wchrg_setMag_f} ${SoC_int}")				
							fi													
						...
						#-------------------------------------------------------------------------------------------					
						if [ $ModuleBalancing -eq 4 ]; then		# 35% .... 100%
							if [ $SoC_int -lt 75 ]; then
								CPOL1_Wchrg_setMag_f=4000
								CPOL1_Wchrg_setMag_f=$(awk '{print $1+$2}' <<<"${CPOL1_Wchrg_setMag_f} ${SoC_int}")				
							fi													
							if ( [ $SoC_int -ge 75 ] && [ $SoC_int -lt 95 ] ); then
								CPOL1_Wchrg_setMag_f=3000
								CPOL1_Wchrg_setMag_f=$(awk '{print $1+$2}' <<<"${CPOL1_Wchrg_setMag_f} ${SoC_int}")				
							fi						
						fi	
						#-------------------------------------------------------------------------------------------	
					...
					if ( [ $SoC_int -ge 99 ] && [ $SoC_int -le 100 ] ); then
						CPOL1_Wchrg_setMag_f=2000
						CPOL1_Wchrg_setMag_f=$(awk '{print $1+$2}' <<<"${CPOL1_Wchrg_setMag_f} ${SoC_int}")				
					fi									
					
					CPOL1_OffsetDuration_setVal=$(awk '{print $1+20}' <<<"${CPOL1_OffsetDuration_setVal}")      #expected 20s for a loop 
					swarmBcSend "CPOL1.Wchrg.setMag.f=$CPOL1_Wchrg_setMag_f" > /dev/null
					swarmBcSend "CPOL1.OffsetDuration.setVal=$CPOL1_OffsetDuration_setVal" > /dev/null
					swarmBcSend "CPOL1.OffsetStart.setVal=$CPOL1_OffsetStart_setVal" > /dev/null				
			...
			# full_capa_module_BMU: -10, - considering that SoC is not 100.
			full_capa_module_BMU_minus=$(awk '{print $1-10}' <<<"${full_capa_module_BMU}")						
			
			
			# Module-Balancing - Phase 2 - Kapazität
			#####################################################################################################################
			echo Module-Balancing 1000 W für 3600 sec '(60min)' oder bis max. $full_capa_module_BMU_minus mAh ..... >> ${_LOGFILE_}
			echo '--------------------------------------------------------------------------' >> ${_LOGFILE_}
			time_current_original_sec_epoch=$(date +%s)
			time_current_sec_epoch=$(date +%s)
			time_offset_sec=3600
			time_limit=$(awk '{print ($1+$2)}' <<<"${time_current_original_sec_epoch} ${time_offset_sec}")
			
			CPOL1_Wchrg_setMag_f=1000							# Power
			CPOL1_offset_time=$(date +%s)						# new starting point for Inverter
			CPOL1_OffsetDuration_setVal=$time_offset_sec		# Preset OffsetDuration
			CPOL1_OffsetStart_setVal=$CPOL1_offset_time				
			...
			while ( [ $time_current_sec_epoch -le $time_limit ] && [ $rem_capa_module_BMU -lt $full_capa_module_BMU_minus ] ); do
					...
					echo '                    ||' BMU-SoC: $SoC_module_BMU '|' $SoC_module_1 $SoC_module_2 $SoC_module_3 $SoC_module_4 $SoC_module_5 $SoC_module_6 $SoC_module_7 $SoC_module_8 $SoC_module_9 $SoC_module_10 '||' BMU_Kapazität: $rem_capa_module_BMU mAh '(' $full_capa_module_BMU mAh ') ||' Zell-Spannung: $U_cell_minV_int mV '|' BMU-Strom: $BMU_current_max_int mA '|' noch max. $time_remaining sec >> ${_LOGFILE_} 
																													
					echo $time '||' Cell_Δ: $U_cell_diff_V mV '|' SoC_Δ: $SoC_module_diff % '|' PVundHH: $PVandHH_int W '|' Capa_% $capa_module_100_int % '|' SoC: $SoC_int % '|' ChargedFlag: $ChargedFlag '|' INV: $CPOL1_Mod '|' INV: $Inv_Request W '|' X '|' Module-Balancing '(2)' >> ${_LOGFILE_}		  				
			...
			#####################################################################################################################
			echo "" >> ${_LOGFILE_}
			echo Module-Balancing 1000 W für 7200 sec '(120min)' oder bis Strom $BMU_current_max_int'mA' kleiner als -500mA ist ..... >> ${_LOGFILE_}
			echo '-------------------------------------------------------------------------------------------------' >> ${_LOGFILE_}
			time_current_original_sec_epoch=$(date +%s)
			time_current_sec_epoch=$(date +%s)
			time_offset_sec=7200
			time_limit=$(awk '{print ($1+$2)}' <<<"${time_current_original_sec_epoch} ${time_offset_sec}")
			...
			while ( [ $time_current_sec_epoch -le $time_limit ] && [ $BMU_current_max_int -gt -500 ] ); do		
					...
					echo $time '||' Cell_Δ: $U_cell_diff_V mV '|' SoC_Δ: $SoC_module_diff % '|' PVundHH: $PVandHH_int W '|' Capa_% $capa_module_100_int % '|' SoC: $SoC_int % '|' ChargedFlag: $ChargedFlag '|' INV: $CPOL1_Mod '|' INV: $Inv_Request W '|' X '|' Module-Balancing '(3)' >> ${_LOGFILE_}		  				
			...
			#####################################################################################################################
			echo "" >> ${_LOGFILE_}
			echo Finales Module-Balancing 1000 W für 3600 sec '(60min)' >> ${_LOGFILE_}
			echo '------------------------------------------------------' >> ${_LOGFILE_}
			time_current_original_sec_epoch=$(date +%s)
			time_current_sec_epoch=$(date +%s)
			time_offset_sec=3600
			...
			while ( [ $time_current_sec_epoch -le $time_limit ] ); do	
					...
					echo $time '||' Cell_Δ: $U_cell_diff_V mV '|' SoC_Δ: $SoC_module_diff % '|' PVundHH: $PVandHH_int W '|' Capa_% $capa_module_100_int % '|' SoC: $SoC_int % '|' ChargedFlag: $ChargedFlag '|' INV: $CPOL1_Mod '|' INV: $Inv_Request W '|' X '|' Module-Balancing '(4)' >> ${_LOGFILE_}		  				
	...
	if ( [ $bmmType == "sony" ] || [ $bmmType == "Synerion48EGen3_ESSG2" ] ); then
		if ( [ $U_cell_diff_V -ge 35 ] && [ $(date +%H) -ge 10 ] && [ $(date +%H) -lt 16 ] && [ $SoC_int -ge 50 ] && [ $SoC_int -le 90 ] || ( [ $CellBalancing -ge 1 ] ) ); then		
			...
			if [ $System_Running -ne $system_running_req ]; then
				...
				pid=$!
				time=$(date +%T)
				echo $time Cell-Balancing Funktion aufgerufen '('PID=$pid')' >> ${_LOGFILE_}
				sleep 50
				Status_Timer_M_activated=0
	...
	while [ $Balancing -eq 1 ] ; do
		...
		echo $time '||' Cell_Δ: $U_cell_diff_V mV '|' SoC_Δ: $SoC_module_diff % '|' PVundHH: $PVandHH_int W '|' Capa_% $capa_module_100_int % '|' SoC: $SoC_int % '|' ChargedFlag: $ChargedFlag '|' INV: $CPOL1_Mod '|' INV: $Inv_Request W '|' Y '|' Cell-Balancing '('PID=$pid')' >> ${_LOGFILE_}		  				
