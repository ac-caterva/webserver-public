...
LOCK_FILE=/tmp/Copy2PiVerteilung.lock 
LOG_FILE=/var/caterva/logs/Copy2PiVerteilung.log 
...
function RsyncCheckForUpdate ()
{
	...
	FILE_TO_COPY=`rsync -n -i --checksum $REPO_BASEDIR/$SOURCE $TARGET | cut -d" " -f2`
...
function RunRSYNC ()
{
	...
	rsync $REPO_BASEDIR/$SOURCE $TARGET
}
