...
# Siegfried Quinger - VA20_2021-03-06_09.00
### END INFO


#-------------------------------------------------------------------------------------------------------------------
version="VA20_2021-03-06_09.00"
#-------------------------------------------------------------------------------------------------------------------
...
#####################################################################################################################
echo '------------------------------------------------------------------------------------------------------------------------' >> ${_LOGFILE_}
echo Module-Balancing Phase 1-4 abgeschlossen - warte 1h. [Der SoC-Wert befindet sich noch nicht bei allen Modulen auf 100%] >> ${_LOGFILE_}
echo Innerhalb der 1h bitte EINE der nachfolgenden Aktionen ausführen:  >> ${_LOGFILE_}
...
echo f] Ruhephase 1h abwarten >> ${_LOGFILE_}
echo '------------------------------------------------------------------------------------------------------------------------' >> ${_LOGFILE_}
fi
...
printf -v time_current_sec_epoch_int %.0f $(date +%s) 			# Unix Epoch Time
time_offset_sec_int=3600		# -> sleep time
time_limit_int=$(awk '{print ($1+$2)}' <<<"${time_current_sec_epoch_int} ${time_offset_sec_int}")
