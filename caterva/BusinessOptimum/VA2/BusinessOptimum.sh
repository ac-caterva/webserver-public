...
#-------------------------------------------------------------------------------------------------------------------
SystemSerial_array=( SN000044 SN000068 SN000109 SN000135 SN000168 SN000173 SN000187 SN000198 SN000230 SN000245 )
#-------------------------------------------------------------------------------------------------------------------
#-------------------------------------------------------------------------------------------------------------------
...
printf -v date_H_int %.0f $(date +%H) # Hour
printf -v date_w_int %.0f $(date +%u) # weekday
if ( ( [ $SoC_module_diff_int -ge 8 ] && [ $SoC_module_diff_int -le 15 ] && [ $date_w_int -eq 6 ] && [ $date_H_int -ge 10 ] && [ $date_H_int -lt 16 ] && [ $SoC_int -ge 60 ] && [ $SoC_int -le 90 ] ) || ( [ $SoC_module_diff_int -ge 11 ] && [ $SoC_module_diff_int -le 15 ] && [ $date_H_int -ge 10 ] && [ $date_H_int -lt 16 ] && [ $SoC_int -ge 60 ] && [ $SoC_int -le 90 ] ) || ( [ $SoC_module_diff_int -ge 16 ] && [ $SoC_module_diff_int -le 35 ] && [ $date_H_int -ge 10 ] && [ $date_H_int -lt 16 ] && [ $SoC_int -ge 50 ] && [ $SoC_int -le 90 ] ) || ( [ $SoC_module_diff_int -ge 36 ] && [ $SoC_module_diff_int -le 60 ] && [ $date_H_int -ge 10 ] && [ $date_H_int -lt 16 ] && [ $SoC_int -ge 25 ] && [ $SoC_int -le 90 ] ) || ( [ $counter_forced_charging_int -ge 3 ] && [ $SoC_int -le 90 ] ) ); then
# Set ModuleBalancing: Ensures that when BusinessOptimum is interrupted / system is shutdown prior to closing of function Module-Balancing, BO will continue with Balancing
...
printf -v CPOL1_offset_time_int %.0f $(date +%s) 		# Unix Epoch Time
CPOL1_OffsetStart_setVal_int=$CPOL1_offset_time_int
fi

if ( [ $SoC_module_diff_int -ge 0 ] && [ $SoC_module_diff_int -le 5 ]  ); then		# when requested due to 3x forced charging or ModuleBalancing
...
printf -v CPOL1_offset_time_int %.0f $(date +%s) 			# Unix Epoch Time  -  new starting point for Inverter
...
# Module-Balancing - Phase 4 - Zeit und 10 Module
#####################################################################################################################
echo "" >> ${_LOGFILE_}
echo Finales Module-Balancing 1500 W für 3600 sec '(60min)' oder mind. bis 10 Module 100% erreicht haben >> ${_LOGFILE_}
echo '--------------------------------------------------------------------------------------------------' >> ${_LOGFILE_}
printf -v time_current_sec_epoch_int %.0f $(date +%s) 			# Unix Epoch Time
time_offset_sec_int=3600
time_limit_int=$(awk '{print ($1+$2)}' <<<"${time_current_sec_epoch_int} ${time_offset_sec_int}")
...
# quit while when 10 of 10 modules reached SoC=1000
while ( [ $time_current_sec_epoch_int -le $time_limit_int ] && [ $SoC_total_int -lt 10000 ] ); do
...
echo "" >> ${_LOGFILE_}
echo "" >> ${_LOGFILE_}
echo Aktueller Status Batteriemodule, nach Module-Balancing Phase 4 '(nach einer weiteren Stunde bzw. bis 100% SoC )' >> ${_LOGFILE_}
echo ---------------------------------------------------------------------------------------------------------------- >> ${_LOGFILE_}
function_Print_Battery_Status_1338


if [ $SoC_total_int -lt 10000 ]; then

# Module-Balancing - Phase 5 (Sleep)
#####################################################################################################################
echo '------------------------------------------------------------------------------------------------------------------------' >> ${_LOGFILE_}
echo Module-Balancing Phase 1-4 abgeschlossen - warte 10h. [Der SoC-Wert befindet sich noch nicht bei allen Modulen auf 100%] >> ${_LOGFILE_}
echo Innerhalb der 10h bitte EINE der nachfolgenden Aktionen ausführen:  >> ${_LOGFILE_}
echo a] BMU Reset: swarmBcResetBmm 2 >> ${_LOGFILE_}
echo b] BMU stromlos: Spannungsversorgung wegnehmen >> ${_LOGFILE_}
echo c] MDEX-Router resetten: Reset oder F20 >> ${_LOGFILE_}
echo d] Inverter - SCU320: HW-Reset an der Einheit [Deckel wegnehmen und Reset-Taste drücken] >> ${_LOGFILE_}
echo e] Komplette Anlage stromlos: FI-Schalter oder F00 >> ${_LOGFILE_}
echo f] Ruhephase 10h abwarten >> ${_LOGFILE_}
echo '------------------------------------------------------------------------------------------------------------------------' >> ${_LOGFILE_}
fi
...
printf -v time_current_sec_epoch_int %.0f $(date +%s) 			# Unix Epoch Time
time_offset_sec_int=36000		# -> sleep time
time_limit_int=$(awk '{print ($1+$2)}' <<<"${time_current_sec_epoch_int} ${time_offset_sec_int}")


# reset when balancing has been completed
rm -f /var/log/ModuleBalancing
...
time_remaining_int=$(awk '{print ($1-$2)}' <<<"${time_limit_int} ${time_current_sec_epoch_int}")
echo $(date +"%Y-%m-%d %T") '||' BMU-SoC: $SoC_module_BMU '|' $SoC_module_1_int $SoC_module_2_int $SoC_module_3_int $SoC_module_4_int $SoC_module_5_int $SoC_module_6_int $SoC_module_7_int $SoC_module_8_int $SoC_module_9_int $SoC_module_10_int '|' Σ: $SoC_total_int '||' BMU_Kapazität: $rem_capa_module_BMU_int mAh '(' $full_capa_module_BMU_int mAh ') ||' Zell-Spannung: $U_cell_minV_int mV '|' BMU-Strom: $BMU_current_max_int mA '|' noch max. $time_remaining_int sec >> ${_LOGFILE_}
echo Module-Balancing '(5)' - EINE der Aktionen ausführen: >> ${_LOGFILE_}
echo a] BMU Reset: swarmBcResetBmm 2 >> ${_LOGFILE_}
echo b] BMU stromlos: Spannungsversorgung wegnehmen >> ${_LOGFILE_}
echo c] MDEX-Router resetten: Reset oder F20 >> ${_LOGFILE_}
echo d] Inverter - SCU320: HW-Reset an der Einheit [Deckel wegnehmen und Reset-Taste drücken] >> ${_LOGFILE_}
...
# Display / Print Battery Status
echo "" >> ${_LOGFILE_}
#echo Aktueller Status Batteriemodule nach Module-Balancing, und Wartezeit von 3 Stunden >> ${_LOGFILE_}
echo ------------------------------------------------------------------------------ >> ${_LOGFILE_}
function_Print_Battery_Status_1338
...
#    # Balancing of Cells when CellBalancing=1 (File /tmp/CellBalancing available)
#====================================================================================================================
#====================================================================================================================
if ( [[ $bmmType == "sony" ]] || [[ $bmmType == "Synerion48EGen3_ESSG2" ]] ); then
printf -v date_H_int %.0f $(date +%H) # Hour
if ( [ $U_cell_diff_V_int -ge 35 ] && [ $date_H_int -ge 10 ] && [ $date_H_int -lt 16 ] && [ $SoC_int -ge 50 ] && [ $SoC_int -le 90 ] || ( [ $CellBalancingRequest_int -ge 1 ] ) ); then
# Set ModuleBalancing: Ensures that when BusinessOptimum is interrupted / system is shutdown prior to closing of function Module-Balancing, BO will continue with Balancing
...
while [ $CellBalancing_int -eq 1 ]; do
rm -f /tmp/balanceBatteryModules.tmp
...
echo "" >> ${_LOGFILE_}
echo "-----------------------------------------------------" >> ${_LOGFILE_}
echo $(date +"%Y-%m-%d %T") '||' Balancing Funktion deaktiviert - System - shutdown >> ${_LOGFILE_}
echo "-----------------------------------------------------" >> ${_LOGFILE_}
echo "" >> ${_LOGFILE_}
sudo shutdown -r now
echo $(date +"%Y-%m-%d %T") '||' '"shutdown -r now"' nicht möglich, deswegen wird forcierter Reboot eingeleitet  >> ${_LOGFILE_}
